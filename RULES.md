...
> 本文件由 `GEMINI.md` 转化而来，作为 Agnes 在此项目中的开发规则。

## 核心指令

- **始终使用中文输出。**
- **编译环境：** 推荐使用 JBR 25（JetBrains Runtime）以支持增强的热重载（HotSwap）功能。
- **环境要求：** 始终处于 PowerShell 环境中，必须使用 PowerShell 命令而非 bash。
...
- **源码参考：** Minecraft 版本为 26.2（2026 年 6 月发布）。执行代码操作前，必须先阅读 `MinecraftSources` 下的源码。若无该目录，请运行 `./gradlew genSources` 并解压至此。
- **热重载配置：**
...
- **模型设计规范：**
  - 每次需要制作模型（包括方块、生物、道具模型等）时，禁止使用无头模式，生成好后使用 `ask_question` 工具询问用户是否满意。
  - **必须严格参考并遵守项目根目录下的 `blockbench.md` 文件中定义的开发规范。**
  - 严格禁止写实主义、平滑圆弧、高面数抛光，必须且仅使用低多边形阶梯状立方体构造，确保符合 Minecraft 的经典原版风格。
- **贴图生成与处理规范：**
  ...
  - **生成贴图规范：** 当要生成工具/武器等贴图时，严格遵守以下规范：
    - 摆放角度：物品整体呈斜向 45 度朝上摆放，类似 Minecraft 快捷栏中工具图标的朝向。
    - 定位规则：手柄/握持端始终位于画面的左下角，工作头部（如镐尖、斧刃、剑尖）指向右上角。
    - 视觉参考：想象一把钻石镐——木棍手柄从左下延伸至中心偏右，镐头在右上区域，整体形成稳定的对角线构图。
    - 关键约束：方向不可偏离，哪怕物品造型不同（如剑的护手、锹的平头），也必须通过旋转和构图使其符合"左下握柄，右上功能端"的斜 45° 法则。
  - **脚本使用示例：**
    ```powershell
...
- **禁止删除文件：** 禁止使用任何方式（包括工具、所有命令、各种脚本等）删除任何文件。需要删除文件时，将其重命名为原来的名字后缀加上 `.bak`，如 `xxx.png` → `xxx.png.bak`。
- **逻辑与伤害计算调试日志：** 将所有逻辑性的计算/获得效果/伤害计算/概率计算等全部使用 `me.tuanzi.util.ModLog` 的 `debug` 方法进行输出，便于调试和寻找问题。

## 项目概述

**tuanzis_mod** 是一个基于 **Fabric Loader** 的 Minecraft 模组，目标版本为 26.1（以 `gradle.properties` 为准）。使用 **Fabric API**，遵循标准 Fabric 模组结构。

- **主入口点：** `me.tuanzi.Tuanzis_mod`
- **数据生成器：** `me.tuanzi.Tuanzis_modDataGenerator`
- **模组 ID：** `tuanzis_mod`
- **许可证：** CC0-1.0
- **技术栈：** Java 25, Gradle, Fabric Loom

## 构建与运行

项目使用 Gradle 包装器（`./gradlew`）管理所有任务。
...
- **命名：** 资源与注册统一使用模组 ID `tuanzis_mod`。
- **日志：** 使用 `Tuanzis_mod.java` 中的静态 `LOGGER` 进行所有日志记录。
- **入口点：** 主要初始化逻辑位于 `Tuanzis_mod#onInitialize`。
- **Mixin：** Mixin 配置文件位于 `src/main/resources/tuanzis_mod.mixins.json`。
- **资源：** 资产文件位于 `src/main/resources/assets/tuanzis_mod/`。
- **数据生成：** 使用 `fabric-datagen` 入口点生成配方、标签和战利品表文件。
...
|------|------|
| `src/main/java` | 源代码 |
| `src/main/resources` | 模组配置、资产和数据 |
| `build.gradle` | 项目依赖与构建配置 |
| `gradle.properties` | 版本与环境设置 |
| `fabric.mod.json` | 模组元数据与入口点 |